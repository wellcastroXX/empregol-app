...
/**
 * Altura → centímetros (o que a API espera, faixa 100–250). Aceita tanto metros
 * ("1.84" / "1,84") quanto centímetros digitados direto ("184"). Retorna null se
 * ficar fora de uma faixa humana plausível — evita o erro cru "≤ 250" da API.
 */
function parseAlturaCm(raw: string): number | null {
  const v = parseFloat(raw.replace(",", ".").replace(/[^\d.]/g, ""));
  if (!v) return null;
  const cm = v < 3 ? Math.round(v * 100) : Math.round(v); // <3 = metros; senão já é cm
...
export function AthleteRegisterScreen() {
  ...
  // Alterna uma posição (máx. 3). A 1ª selecionada é tratada como principal.
  const togglePosicao = (value: Position) =>
    setForm((prev) => {
      const has = prev.posicoes.includes(value);
      if (has)
        return { ...prev, posicoes: prev.posicoes.filter((p) => p !== value) };
      if (prev.posicoes.length >= MAX_POSICOES) return prev;
      return { ...prev, posicoes: [...prev.posicoes, value] };
    });
  ...
  const validateStep = (): string | null => {
    if (step === 0) {
      ...
      if (!toIsoDate(form.dataNascimento))
        return "Data de nascimento inválida.";
      if (form.naturalidade.trim().length < 2)
    ...
    if (step === 1) {
      ...
      if (form.posicoes.length === 0) return "Selecione ao menos uma posição.";
      if (!form.peDominante) return "Selecione o pé dominante.";
      if (parseAlturaCm(form.alturaM) === null)
        return "Altura inválida. Use metros (ex.: 1.84).";
    ...
    if (step === 2) {
      ...
      if (!Number(unmask(form.baseSalarial)))
        return "Informe a base salarial pretendida.";
    }
    return null;
  };
  ...
  const submit = async () => {
    ...
    const alturaCm = parseAlturaCm(form.alturaM)!;
    setLoading(true);
    try {
      await register({
        role: "athlete",
        ...
        genero: form.genero!,
        posicao: form.posicoes[0], // principal (compat com quem usa 1 só)
  ...
  const HEADERS = [
    {
      eyebrow: "I D E N T I D A D E · L E G A L",
      title: "Seus dados pessoais",
      subtitle:
        "Dados oficiais usados pra validar tua conta e firmar contratos.",
    },
    {
      eyebrow: "P E R F I L · E S P O R T I V O",
      title: "Seu perfil\nesportivo.",
      subtitle: undefined,
    },
    {
      eyebrow: "S T A T U S · & · P R E T E N S Ã O",
      title: "Disponibilidade e\npretensão.",
      subtitle:
        "Esses campos aparecem pros clubes na tua vitrine. Edita quando quiser.",
    },
  ];
          ...
          {step === 0 && (
            <>
              <TextField
                ...
                placeholder="Lucas Henrique da Silva"
              />
              <TextField
                label="CPF"
                required
                mono
                hint="Validado online"
              ...
              <TextField
                ...
                onChangeText={(v) => set("naturalidade", v)}
                placeholder="Cidade - UF"
              />
              <TextField
                label="E-MAIL"
                required
                hint="Vamos verificar"
                autoCapitalize="none"
            ...
            <>
              ...
              <Field label="POSIÇÃO">
                <Text variant="sm" color={colors.fgMuted}>
                  Selecione até {MAX_POSICOES} · {form.posicoes.length}/
                  {MAX_POSICOES}
                </Text>
                <ChipGroup
                  options={POSITION_CHIPS}
                  multiple
                  values={form.posicoes}
              ...
              <View style={styles.row}>
                ...
                <View style={styles.col}>
                  <TextField
                    ...
                    keyboardType="number-pad"
                    value={form.pesoKg}
                    onChangeText={(v) => set("pesoKg", v.replace(/\D/g, ""))}
                    placeholder="78"
                  />
                </View>
              </View>
              ...
              <TextField
                label="ÚLTIMO CLUBE"
                hint="Profissional, base ou amador"
                value={form.ultimoClube}
          ...
          {step === 2 && (
            <>
              ...
              <Field label="A G E N C I A M E N T O">
                <ChipGroup
                  options={AGENCY_CHIPS}
                  value={form.agenciamento}
                  onChange={(v) => set("agenciamento", v)}
                  size="lg"
                />
              </Field>
              <TextField
                label="BASE SALARIAL PRETENDIDA"
                hint="Visível p/ clubes"
                mono
                suffix="BRL"
                keyboardType="number-pad"
                value={form.baseSalarial}
                onChangeText={(v) => set("baseSalarial", maskThousands(v))}
                placeholder="12.000"
              />
              <TextField
                label="LINK DE VÍDEOS"
                hint="Visível p/ clubes"
                autoCapitalize="none"
                value={form.videos}
                onChangeText={(v) => set("videos", v)}
                placeholder="youtube.com"
              />
              <TextField
                label="LINK DA SUA REDE SOCIAL"
                hint="Visível p/ clubes"
                autoCapitalize="none"
                value={form.redeSocial}
                onChangeText={(v) => set("redeSocial", v)}
                placeholder="@instagram"
              />
              <View style={styles.infoBox}>
                <Text variant="sm" color={colors.fg}>
                  Perfis com vídeo + base salarial recebem em média 3× mais
                  propostas.
                </Text>
              </View>
            </>
...
function DispButton({
  ...
  return (
    ...
    >
      ...
      <Text
        style={styles.dispName}
        color={active ? palette.giz : palette.tinta}
      >
        {label}
      </Text>
    </Pressable>
